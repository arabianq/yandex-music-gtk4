...
class YamusicClient:
    ...
    def get_liked_playlist(self):
        ...
        return {"title": title, "kind": kind, "owner_id": owner_id, "cover": cover}
    ...
    def get_user_playlists(self):
        ...
        for i in range(len(raw_playlists)):
            ...
            playlist = {"title": title, "kind": kind, "owner_id": owner_id, "cover": cover}
    ...
    def get_all_user_playlists(self, use_cache=True):
        if has_internet() and self.client:
            ...
            if use_cache:
                json.dump(playlists, open(f"{CACHE_DIR}/user_playlists.json", "w", encoding="utf-8"), indent=4,
    ...
    def get_track_url(self, track):
        while True:
            try:
                url = sorted(self.client.tracks_download_info(track.id, True), key=lambda x: x["bitrate_in_kbps"],
    ...
    def fetch_tracks_from_playlist(self, playlist, use_cache=True):
        def cache_tracks(tracks_to_cache):
            ...
            reformatted_tracks = [
                {"title": track.title, "artists": [artist.name for artist in track.artists], "id": track.id,
                 "available": os.path.exists(f"{CACHE_DIR}/downloaded_tracks/{track.id}.mp3"), } for track in
                tracks_to_cache]

            # del cache[f"{playlist['owner_id']}_{playlist['kind']}"]
            cache[f"{playlist['owner_id']}_{playlist['kind']}"] = reformatted_tracks

            json.dump(cache, open(f"{CACHE_DIR}/cached_tracks.json", "w", encoding="utf8"), indent=4, sort_keys=True,
                      ensure_ascii=False)
        ...
        if has_internet() and self.client:
            ...
            tracks = json.load(open(f"{CACHE_DIR}/cached_tracks.json", "r", encoding="utf8"))[
                f"{playlist['owner_id']}_{playlist['kind']}"]
            for track in tracks:
                del track["available"]
